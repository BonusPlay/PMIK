...
static bool read_register16(BMP280_HandleTypedef* dev, uint8_t reg, uint16_t* value)
{
    // read bit 7 high
...
static bool read_data(BMP280_HandleTypedef* dev, uint8_t reg, uint8_t* value, uint8_t len)
{
    // read, bit 7 high
    uint16_t tx_buff = (reg | 0x80);
...
static bool write_register8(BMP280_HandleTypedef* dev, uint8_t reg, uint8_t value)
{
    // write, bit 7 low
    uint8_t tx_buff[2];
    tx_buff[0] = (reg & ~0x80);
